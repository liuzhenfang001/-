...
## 7.K-means
### 缺点：
1）受初始化和离群点的影响，每次结果都不稳定  
2）结果通常不是全局最优，而是局部最优  
3）无法很好解决数据簇分布差异比较大的情况，例如一类是另一类样本数的100倍  
4）不太适用于离散特征值  
### 优点：
 1）复杂度O(KNt)，几乎是线性的，对大数据集具有很好的伸缩性  
 2）一般情况达到局部最优，已经可以满足分类的要求
### 数据归一化和离群点处理
均值和方差大的特征会对聚类产生非常大的决定性影响。未做归一化处理无法直接参与运算。  
离群点和噪声会对均值产生较大的影响，导致中心偏移，所以需要预处理数据

### 合理选择K值
#### 手肘法
就是尝试K=0,1,...N,画出“K-误差平方和”图，选择拐点明显的K值，此方法是经验方法，不够自动化
#### Gap statstic
#### 核函数
通过非线性变化，将特征映射到更高维的特征空间，使得在原始非凸特征空间的无法聚类问题 在高维空间中可聚类，或者增加聚类的可能性。例如同心圆的数据分布。
